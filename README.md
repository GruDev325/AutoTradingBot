...
-   **source_coin_split_fraction** - Uses this fraction of source coin to buy the next destination coin if source balance is above `minimum_balance_threshold_for_split` (Only valid for parallel transactions strategy)
-   **minimum_balance_threshold_for_split** - The source coin will be split only if it has balance (in bridge coin) above this. (Only valid for parallel transactions strategy)
-   **significant_balance_threshold** - The coin will not be bought if it has more than this balance (in bridge coin). (only valid for parallel transactions strategy)
-   **minimum_balance_threshold_for_scouting** - The current coin should have this much balance (in terms of Bridge coin) to be eligible for scouting (only valid for parallel transactions strategy)
